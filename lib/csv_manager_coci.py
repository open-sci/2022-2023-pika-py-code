...
class CSVManager(object):
    ...
    def __init__(self, output_path: str = None, line_threshold=10000, count: bool = False, low_memory: bool = False):
    ...
    def __get_existing_files(self) -> list:
        ...
        if exists(self.output_path):
            for cur_dir, _, cur_files in walk(self.output_path):
                for cur_file in tqdm(cur_files):
    ...
    @staticmethod
    def count_element_in_column(file_or_dir_path: str, key: str, element: str, line_threshold: int = 10000):
        result = list()
        if exists(file_or_dir_path):
            file_to_process = []
            if isdir(file_or_dir_path):
    ...
    def __load_all_csv_files(file_to_process, fun, line_threshold, **params):
        result = []
        header = None
        for csv_path in tqdm(file_to_process):
    ...
    def __load_csv(self):
        for file in tqdm(self.existing_files):
            with open(file, 'r', encoding='utf-8') as f:
                reader = DictReader(f, delimiter=",")
                for row in reader:
                    self.data.setdefault(row['citing'], set())
                    self.data[row['citing']].add(row['cited'])
    ...
    def __low_memory_load(self, csv_string: str):
        ...
        for row in csv_metadata:
            ...
            self.data[cur_id].add(row['cited'])
